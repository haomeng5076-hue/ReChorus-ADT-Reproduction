...
class BaseRunner(object):
	...
	def __init__(self, args):
		...
		self.main_metric = '{}@{}'.format(self.metrics[0], self.topk[0]) if not len(args.main_metric) else args.main_metric
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1) -> float:
		...
		for batch in tqdm(dl, desc=f'Epoch {epoch}/{self.epoch}', leave=False, ncols=80, ascii=True, file=sys.stdout):
	...
	def predict(self, dataset: BaseModel.Dataset, save_prediction: bool = False) -> np.ndarray:
		...
		# 【修复】加上 file=sys.stdout，解决进度条换行问题
		for batch in tqdm(dl, leave=False, ncols=80, mininterval=1, desc='Predict', ascii=False, file=sys.stdout):
